import { InvitationResponse } from '../../viewModels/interfaces';
...
@Injectable()
export class EmailConfirmationsProvider {
  ...
  sendAcceptInvitation(token: string): Observable<InvitationResponse> {
    return this.http.get<InvitationResponse>(
      `${environment.restServiceRoot}${this.acceptReserveRestPath}` + token,
    );
  }

  sendRejectInvitation(token: string): Observable<InvitationResponse> {
    return this.http.get<InvitationResponse>(
      `${environment.restServiceRoot}${this.rejectReserveRestPath}` + token,
    );
  }

  sendCancelBooking(token: string): Observable<InvitationResponse> {
    return this.http.get<InvitationResponse>(
      `${environment.restServiceRoot}${this.cancelReserveRestPath}` + token,
    );
  }

  sendCancelOrder(token: string): Observable<boolean> {
    return this.http.get<boolean>(
      `${environment.restServiceRoot}${this.cancelOrderRestPath}` + token,
    );
  }
}
